...
from glasses.dependencies import get_k8_client
...
class UpdateableListView(Widget):
    ...
    async def update(
        ...
        for cmd in commands:
            self._listview.append(ListItem(Label(f" \[{cmd.value}]"), id=cmd.name))
...
class SlideView(Widget):
    ...
    def __init__(self) -> None:
        super().__init__()
        self.history: list[BaseK8] = [Cluster("teset_cluster", get_k8_client())]
